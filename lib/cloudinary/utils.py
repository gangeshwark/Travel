...
def generate_transformation_string(**options):
    ...
    if any(isinstance(bs, dict) for bs in base_transformations):
        recurse = lambda bs: generate_transformation_string(**bs)[0] if isinstance(bs, dict) else \
        generate_transformation_string(transformation=bs)[0]
    ...
    if isinstance(border, dict):
        border = "%(width)spx_solid_%(color)s" % {"color": border.get("color", "black").replace("#", "rgb:"),
                                                  "width": str(border.get("width", 2))}

    flags = ".".join(build_array(options.pop("flags", None)))
    dpr = options.pop("dpr", cloudinary.config().dpr)

    params = {"w": width, "h": height, "t": named_transformation, "b": background, "co": color, "e": effect, "c": crop,
              "a": angle, "bo": border, "fl": flags, "dpr": dpr}
    for param, option in {"q": "quality", "g": "gravity", "p": "prefix", "x": "x",
                          "y": "y", "r": "radius", "d": "default_image", "l": "overlay", "u": "underlay",
                          "o": "opacity",
                          "f": "fetch_format", "pg": "page", "dn": "density", "dl": "delay",
                          "cs": "color_space"}.items():
        params[param] = options.pop(option, None)

    transformation = ",".join(
        sorted([param + "_" + str(value) for param, value in params.items() if (value or value == 0)]))
...
def api_sign_request(params_to_sign, api_secret):
    to_sign = "&".join(
        sorted([(k + "=" + (",".join(v) if isinstance(v, list) else str(v))) for k, v in params_to_sign.items() if v]))
...
def cloudinary_url(source, **options):
    ...
    if source_to_sign.find("/") >= 0 and not re.match(r'^https?:/', source_to_sign) and not re.match(r'^v[0-9]+',
                                                                                                     source_to_sign) and not version:
        version = "1"
...
def smart_escape(string):
    pack = lambda m: to_bytes(
        '%' + "%".join(["%02X" % x for x in struct.unpack('B' * len(m.group(1)), m.group(1))]).upper())
