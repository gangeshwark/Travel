...
def resources(**options):
    ...
    return call_api("get", uri,
                    only(options, "next_cursor", "max_results", "prefix", "tags", "context", "moderations", "direction",
                         "start_at"), **options)
...
def resources_by_tag(tag, **options):
    ...
    return call_api("get", uri,
                    only(options, "next_cursor", "max_results", "tags", "context", "moderations", "direction"),
                    **options)
...
def resource(public_id, **options):
    ...
    return call_api("get", uri,
                    only(options, "exif", "faces", "colors", "image_metadata", "pages", "phash", "coordinates",
                         "max_results"), **options)
...
def update(public_id, **options):
    ...
    type = options.pop("type", "upload")
    uri = ["resources", resource_type, type, public_id]
    upload_options = only(options, "moderation_status", "raw_convert", "ocr", "categorization", "detection",
                          "similarity_search", "background_removal")
    if "tags" in options: upload_options["tags"] = ",".join(utils.build_array(options["tags"]))
    if "face_coordinates" in options: upload_options["face_coordinates"] = utils.encode_double_array(
        options.get("face_coordinates"))
    if "custom_coordinates" in options: upload_options["custom_coordinates"] = utils.encode_double_array(
...
def delete_resources_by_prefix(prefix, **options):
    ...
    type = options.pop("type", "upload")
    uri = ["resources", resource_type, type]
    return call_api("delete", uri, dict(only(options, "keep_original", "next_cursor", "invalidate"), prefix=prefix),
                    **options)
...
def delete_all_resources(**options):
    ...
    return call_api("delete", uri, dict(only(options, "keep_original", "next_cursor", "invalidate"), all=True),
                    **options)
...
def transformation_string(transformation):
    return transformation if isinstance(transformation, str) else \
    cloudinary.utils.generate_transformation_string(**transformation)[0]
